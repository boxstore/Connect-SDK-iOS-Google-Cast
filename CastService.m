...
- (void)rewindWithSuccess:(SuccessBlock)success failure:(FailureBlock)failure
{
    ...
    NSTimeInterval position = self.castMediaControlChannel.approximateStreamPosition;
    position -= 30;
    if (position < 0) {
        position = 0;
    }
    ...
    @try
    {
        result = [self.castMediaControlChannel seekToTimeInterval:position];
    }
    @catch (NSException *exception)
    {
        // this exception will be caught when trying to send command with no video
...
- (void)fastForwardWithSuccess:(SuccessBlock)success failure:(FailureBlock)failure
{
    ...
    NSTimeInterval position = self.castMediaControlChannel.approximateStreamPosition;
    position += 30;
    if (position > self.castMediaControlChannel.mediaStatus.mediaInformation.streamDuration) {
        position = self.castMediaControlChannel.mediaStatus.mediaInformation.streamDuration - 1;
    }
    NSInteger result;
    @try
    {
